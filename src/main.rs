...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    // only write background on the first frame
    if app.elapsed_frames() == 1 {
        draw.background().color(BLACK);
    }
    // don't try to erase the past, give a hint of the past by drawing a transparent black rectangle
    // over animation. Get fade to black effect for free
